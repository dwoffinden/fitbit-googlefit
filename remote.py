...
class Remote:
    ...
    def SyncFitbitIntradayToGoogleFit(self, dataType, date_stamp):
        ...
        interday_raw = self.ReadFromFitbit(
            self.fitbitClient.intraday_time_series, res_path, base_date=date_stamp, detail_level=detail_level
        )
        try:
    ...
    def SyncFitbitActivitiesToGoogleFit(self, start_date="", callurl=None):
        ...
        if len(startTimeMillis) > 0:
            print(
                "Synced {} exercises between : {} -- {}".format(
                    len(activities),
